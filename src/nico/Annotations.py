...
def find_anchor_cells_between_ref_and_query(
    ...
    inputvar['adata_query']=sct_ad_sp
    inputvar['fmnn']=fmnn
...
number_of_iteration_to_perform_celltype_annotations=3,cmap=plt.cm.get_cmap('jet'),saveas='pdf',transparent_mode=False,showit=True,figsize=(15,6)):
    ...
    #fig.tight_layout()
    print(f"The figures are saved: {fig_save_path}tissue_and_umap_with_all_celltype_annotations.{saveas}")
    fig.savefig(f"{fig_save_path}tissue_and_umap_with_all_celltype_annotations.{saveas}",bbox_inches='tight',transparent=transparent_mode,dpi=300)
    if showit:
        pass
    else:
        plt.close('all')
